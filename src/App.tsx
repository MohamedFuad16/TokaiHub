...
export type Screen = 'home' | 'course' | 'schedule' | 'settings' | 'assignments' | 'editProfile';
...
const sidebarNav = [
  { screen: 'home' as Screen, icon: Home, labelEn: 'Home', labelJp: 'ホーム' },
  { screen: 'schedule' as Screen, icon: Calendar, labelEn: 'Schedule', labelJp: 'スケジュール' },
  { screen: 'assignments' as Screen, icon: ClipboardList, labelEn: 'Assignments', labelJp: '課題' },
  { screen: 'settings' as Screen, icon: Settings, labelEn: 'Settings', labelJp: '設定' },
];
...
export default function App() {
  ...
  useEffect(() => {
    async function checkAuthStatus() {
      try {
        ...
        // Populate profile with cognito attributes. 
        // In reality, courseIds and GPA would come from a database, but we stub them for now.
        setUserProfile({
  ...
  const handleSidebarNavigate = (s: Screen) => {
    setHistory([{ screen: s }]);
  };
        ...
        <nav className="flex-1 px-4 py-6 space-y-1">
          {sidebarNav.map(item => {
            ...
            return (
              <button
                key={item.screen}
                onClick={() => handleSidebarNavigate(item.screen)}
      ...
      <main className={`flex-1 h-full relative overflow-hidden transition-colors duration-500 ${isDark ? 'bg-gray-900 text-white' : 'bg-white text-gray-900'}`}>
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {currentScreen === 'settings' && <TokaiSettings {...screenProps} onDevSkipChange={handleDevSkipChange} />}

    ...
    <PageShell {...props} title={tx.title} subtitle={since(reports.cachedAt, lang) || undefined} onRefresh={() => { reports.refresh(); exams.refresh(); }} refreshing={reports.loading || exams.loading}>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <section>
          ...
          {!reports.data ? (reports.error ? <LoadError error={reports.error} isDark={isDark} lang={lang} onRetry={reports.refresh} /> : <Loading text={tx.loading} isDark={isDark} rows={2} />) : allReports.length === 0 ? <Empty text={tx.noReports} isDark={isDark} /> : (
                  ...
                  <Card isDark={isDark} className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <div className="font-semibold text-sm break-words [overflow-wrap:anywhere]"><Linked text={r.title} isDark={isDark} /></div>
          ...
          <a href="https://tips.u-tokai.ac.jp/campusweb/campussquare.do?_flowId=RMW0001000-flow" target="_blank" rel="noopener noreferrer" className={`mt-2 h-10 inline-flex items-center gap-1.5 text-xs font-bold hover:underline underline-offset-2 ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
            <ExternalLink className="w-3.5 h-3.5" />{tx.submit}
          </a>
        </section>

        <section>
          <SectionTitle right={tt.timetable ? <span className={`text-xs font-semibold ${muted}`}>{termLabel(term, year, lang)}</span> : undefined}><CalendarClock className="w-4 h-4" />{tx.exams}</SectionTitle>
          {!exams.data ? ((exams.error ?? tt.error) ? <LoadError error={(exams.error ?? tt.error)!} isDark={isDark} lang={lang} onRetry={() => { tt.refresh(); exams.refresh(); }} /> : <Loading text={tx.loading} isDark={isDark} rows={2} />) : exams.data.length === 0 ? <Empty text={tx.noExams} isDark={isDark} /> : (
            <div className="space-y-2">
              {exams.data.map((e, i) => (
                ...
                <Card isDark={isDark} className="flex gap-4 p-4">
                  ...
                  <div className="min-w-0">
                    ...
                    {e.notes && <div className="text-xs mt-1 font-semibold break-words [overflow-wrap:anywhere]"><Linked text={e.notes} isDark={isDark} /></div>}
                  </div>
                </Card>
                </motion.div>
              ))}

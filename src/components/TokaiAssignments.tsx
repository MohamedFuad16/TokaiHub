  ...
  return (
    <div className="h-full relative flex flex-col">
      ...
      <header className="flex justify-between items-center p-6 pt-12 shrink-0">
        <div className="flex items-center gap-4">
          <button 
            ...
            className={`w-10 h-10 rounded-full border ${borderClass} flex items-center justify-center transition-colors ${isDark ? 'hover:bg-gray-800' : 'hover:bg-gray-50'}`}
          >
            <Menu className="w-5 h-5" />
          </button>
          <h1 className="text-[28px] font-bold tracking-tight">{t[lang].assignments}</h1>
        </div>
        <button 
          onClick={() => setTimeout(goBack, 150)}
          className="w-10 h-10 rounded-full bg-brand-yellow flex items-center justify-center hover:bg-yellow-400 transition-colors active:scale-95"
        >
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden p-6 pt-2">
        <motion.div variants={containerVariants} initial="hidden" animate="show" className={`${bgClass} rounded-[40px] p-6`}>
          <motion.div variants={itemVariants} className="flex items-center gap-3 mb-8">
            <div className="w-12 h-12 bg-brand-black text-white rounded-full flex items-center justify-center">
              <Plus className="w-6 h-6" />
            </div>
            <h2 className="text-2xl font-bold leading-tight">{t[lang].addAssignment}</h2>
          </motion.div>

          <motion.form variants={itemVariants} className="space-y-5" onSubmit={(e) => { e.preventDefault(); onNavigate('schedule'); }}>
            ...
            <button 
              type="submit"
              className="w-full bg-brand-black text-white rounded-full py-4 font-bold text-lg hover:bg-gray-800 transition-colors mt-8 shadow-xl"
            >
              {t[lang].submit}
            </button>
          </motion.form>

...
export interface Prefs {
  /** Per graduation section (e.g. "IV", "V"). */
  sections: Record<string, { delivery: DeliveryPref; assessment: AssessmentPref }>;
...
export const DEFAULT_PREFS: Prefs = { sections: {}, continueSeries: true, lighter: false, avoidFirstPeriod: false };

export type Reason = 'remote' | 'in_person' | 'assignment' | 'exam' | 'continues' | 'light' | 'required' | 'required_elective' | 'first_period';
...
export function scoreOf(o: Offering, prefs: Prefs): Ranked {
  const p = prefs.sections[o.section ?? ''] ?? { delivery: 'any', assessment: 'any' };
  let score = 0;
  ...
  if (p.delivery === 'remote') {
    if (remote) { score += 3; reasons.push('remote'); } else if (o.delivery.kind === 'hybrid') score += 1; else if (o.delivery.kind === 'in_person') score -= 2;
  } else if (p.delivery === 'in_person') {
    if (o.delivery.kind === 'in_person') { score += 2; reasons.push('in_person'); } else if (remote) score -= 1;
  }
  const { examShare, assignmentShare } = o.assessment;
  if (p.assessment === 'assignment') {
    score += 4 * assignmentShare - 2 * examShare;
    if (o.assessment.style === 'assignment') reasons.push('assignment');
  } else if (p.assessment === 'exam') {
  ...
  if (prefs.avoidFirstPeriod && o.slots.some(s => s.period === 1)) { score -= 1.5; reasons.push('first_period'); }
...
export interface Plan { picks: Ranked[]; credits: number; target: number; budget: number }

/**
 * This term's plan: first the required courses that can be taken now, then the best-scoring
 * courses that still count toward a section the student needs (following the graduation lines,
...
export function buildPlan(data: RecommendData, prefs: Prefs, extra: PlanItem[] = []): Plan {
  ...
  const ranked = data.offerings.filter(usable).map(o => scoreOf(o, prefs)).sort((a, b) => b.score - a.score);
  const picks: Ranked[] = [];
  ...
  return { picks, credits, target: data.credits.target, budget };
}
